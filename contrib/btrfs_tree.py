...
import drgn
from drgn import cast, offsetof
...
def _btrfs_bin_search(eb, p, item_size, key: BtrfsKey):
    ...
    while low < high:
        ...
        disk_objectid, disk_type, disk_offset = _btrfs_disk_key_struct.unpack(
            read_extent_buffer(eb, offset, 17)
        )
...
def btrfs_leaf_items(eb):
    ...
    for i in range(nritems):
        ...
        items.append(
            BtrfsItem(
                BtrfsKey(raw_item[0], raw_item[1], raw_item[2]),
                raw_item[3],
                raw_item[4],
            )
        )
...
def parse_extent_buffer(eb):
    ...
    if header.level == 0:
        ...
        ptrs_buf = read_extent_buffer(
            eb, BtrfsHeader.sizeof(), header.nritems * BtrfsKeyPtr.sizeof()
        )
        return BtrfsNode(
            header,
            [
...
def print_extent_buffer(eb):
    ...
    print(
        f"node {node.header.bytenr} level {node.header.level} items {node.header.nritems} generation {node.header.generation} owner {node.header.owner}"
    )
    ...
    for i, ptr in enumerate(node.ptrs):
        print(
            f"\tptr {i} key ({ptr.key.objectid}, {ptr.key.type}, {ptr.key.offset}) block {ptr.blockptr} gen {ptr.generation}"
        )
